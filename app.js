...
const CONFIG = {
    ...
    REDIRECT_URI: "https://happy-home-e120.onrender.com/auth/callback" // 이 주소가 구글 콘솔과 똑같아야 함
};

// Gemini 초기화 (안정적인 버전 사용)
        ...
        <div style="text-align: center; margin-top: 50px; font-family: sans-serif;">
            ...
            <div style="display: flex; flex-direction: column; align-items: center; gap: 15px; margin-top: 30px;">
                <a href="https://accounts.google.com/o/oauth2/v2/auth?client_id=${CONFIG.GOOGLE_ID}&redirect_uri=${CONFIG.REDIRECT_URI}&response_type=code&scope=email profile&state=google_${user_key}" 
                   style="width: 220px; padding: 12px; background: white; border: 1px solid #ccc; text-decoration: none; color: black; border-radius: 8px; font-weight: bold; display: block;">Google 로그인</a>
...
app.get('/auth/callback', (req, res) => {
    ...
    res.send("<script>alert('인증 성공! 카톡으로 돌아가서 [인증확인]을 누르세요.'); window.close();</script>");
});
...
app.post('/kakao-auth', async (req, res) => {
    try {
        ...
        if (uttr.includes("인증")) {
            if (authList[userKey]) {
                return res.status(200).json({
                    version: "2.0",
                    template: { outputs: [{ simpleText: { text: "✅ 인증이 완료되었습니다! 이제 자유롭게 대화하세요." } }] }
                });
            }
        }

        // 미인증 시 로그인 유도 (Thumbnail 필수 포함)
        if (!authList[userKey]) {
            return res.status(200).json({
                version: "2.0",
                template: {
                    outputs: [{
                        basicCard: {
                            title: "가족 인증이 필요합니다",
                            description: "로그인 후 아래 [✅인증확인✅] 버튼을 눌러주세요.",
        ...
        return res.status(200).json({
            version: "2.0",
            template: { outputs: [{ simpleText: { text: "잠시 연결이 원활하지 않아요. 다시 시도해 주세요!" } }] }
        });
    }
});

const PORT = process.env.PORT || 10000;
app.listen(PORT, '0.0.0.0', () => console.log(`서버 가동 중: ${PORT}`));
